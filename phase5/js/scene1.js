...
var screens = { '1': {
  'boundaries': {
    ...
    'corridor': { polygon: [297, 1041, 501, 1067, 532, 1026, 324, 1024, 297, 1041] },
    'exit-corridor': { polygon: [510, 1031, 590, 1029, 635, 1045, 652, 1094, 584, 1118, 509, 1110, 466, 1081, 522, 1031, 510, 1031] },
    ...
    'stairs': { 'polygon': [462, 672, 452, 619, 473, 573, 519, 541, 553, 533, 621, 525, 681, 522, 758, 518, 787, 513, 836, 501, 891, 494, 996, 500, 1019, 627, 1021, 704, 1001, 755, 986, 807, 980, 871, 709, 828, 567, 765, 476, 696, 467, 680, 462, 672] },
    ...
    'fallunderwaterfall': {
      ...
      enter: function () {
        ...
        setRestore(restorePoints.waterfall);
        doActions(
          800,
  ...
  'regions': {
    spacebottomleft: {
      nouns: ["space", "outside", "void", "galaxy", "stars", "star", "hyperspace", "around"],
    ...
    corridor: {
      nouns: ["corridor", "tunnel", "wall", "window", "windows", "port", "space port"],
    ...
    elevatorpad: {
      nouns: ["pad", "elevator", "lift", "floor"],
      verbs: {
        'take': "It won't budge. What did you expect?"
      },
    ...
    'corridor-panel': {
      nouns: ["panel", "button", "elevator button", "arrow", "green arrow"],
      'hand': elevatorGoesUp,
    ...
    buckazoid: {
      ...
      hand: function () {
        if (!Inventory.has('buckazoid')) {
          ...
          $('.buckazoid:last-child').hide();
          $('#cluckcontainer').html('<iframe id="cluck" src="cluck.html" frameborder="0"></iframe>');
          if ($('.arcade').hasClass('opening')) {
            showDialog(dialogs.screen1.buckazoidhandagain);
          } else {
            showDialog(dialogs.screen1.buckazoidhand);
          }
        }
        else {
          doActions(dialogs.screen1.buckazoidhand2);
        }
      },
      hotspot: [903, 692], distance: 100,
      polygon: [902, 672, 859, 701, 864, 712, 905, 713, 947, 709, 952, 704]
    },
    leftsidefronds: {
    ...
    arcade: {
      nouns: ["arcade", "cabinet", "machine", "game", "computer", "arcade machine"],
    ...
    arcadejoystick: {
      nouns: ["joystick", "stick", "arcade joystick"],
    ...
    arcadepad: {
      nouns: ["controls", "buttons", "button", "control panel"],
    ...
    arcadeslot: {
      ...
      hand: function () {
        if (Inventory.has('buckazoid') && !cabinetOpened) {
          Inventory.drop('buckazoid');
          playCluck();
        }
        else if (Inventory.has('buckazoid') && cabinetOpened) {
          Inventory.drop('buckazoid');
          playCluck();
        }
        else {
          showDialog(dialogs.screen1.arcadeslotnocoin);
        }
      },
    ...
    'waterfall-panel': {
      nouns: ["panel", "button", "slot", "wall panel", "wall", "slit"],
      use: ['keycard'],
      'hand': function () {
        if (!cabinetOpened) {
    ...
    'control-panel': {
      nouns: ["panel", "controls", "control panel", "dashboard", "gauges", "dials", "console", "console panel", "buttons"],
    ...
    'mark': {
      nouns: ["mark", "andromedan", "guy", "alien", "man", "mark crowe", "dude", "guys"],
      'look': dialogs.screen1.marklook,
      'hand': dialogs.screen1.markhand,
      'polygon': [1706, 464, 1762, 678, 1864, 639, 1779, 455]
    },
    'scott': {
      nouns: ["scott", "andromedan", "guy", "alien", "man", "scott murphy", "dude", "guys"],
...
function fallInTheWater() {
  ...
  doActions(
    ...
    function () {
      ...
      setRestore(restorePoints.rocks);
      setTimeout(function () {
...
function deathByAirlock() {
  ...
  setRestore(restorePoints.corridor);
  doActions(
    6000,
...
function playCluck() {
  ...
  if ($('.arcade').hasClass('opening')) {
    doActions(
      dialogs.screen1.cluckstartagain,
      function() {
        $('#cluck').addClass('preshow');
        setTimeout(cluckStart, 300);
      }
    );
  }
  else {
...
function cluckEnd() {
  ...
  $('#cluck').remove();
  if ($('.arcade').hasClass('opening')) {
    doActions(dialogs.screen1.cluckendagain, Story.disable);   
  }
  else {
...
function addBuckazoids() {
  if ($('.buckazoids .buckazoid').length > 0) return;
  var num = 7;
  for (var i = 0; i < num; i++) {
    for (var j = num; j >= 0; j--) {
      var buck = $("<div/>").addClass('layer buckazoid');
      if (Math.random() < .5) buck.addClass('bronze');
      else if (Math.random() < .5) buck.addClass('silver');
      else buck.addClass('gold');
      buck.css({
        'margin-top': 4 * i + (Math.random() * 10),
        'margin-left': (i * j + (Math.random() * 6)) * (i%2==0 ? -1 : 1)
      });
      $(".buckazoids").append(buck);
    }
  }
}
...
var restorePoints = {
  ...
  waterfall: function () {
    Story.disable();
    hideDialog();
    $('#avatar').removeClass('drown');
    $('.keycard').show();
    $('.plant').removeClass('frame4');
    setCursorState(CursorStates.walk);
    avatar.position(1215, 413).show();
    $("#avatar").css('display', '');
    unpause();
    cycle();
  },
  rocks: function () {
    $('.blood').removeClass('appear').hide();
    $(".shark").removeClass('disappear');
    restorePoints.waterfall();
  }
}